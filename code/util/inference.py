...
def test_single_volume_ensemble(image, label, net, classes, patch_size=[224, 224]):
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            net.eval()
            with torch.no_grad():
                out1 = torch.softmax(net(input)['seg'], dim=1)
                out2 = torch.softmax(net(input)['seg_tf'], dim=1)
        ...
        with torch.no_grad():
            out = torch.argmax(torch.softmax(
                ((net(input)['seg'] + net(input)['seg_tf']) / 3), dim=1), dim=1).squeeze(0)
            prediction = out.cpu().detach().numpy()
    metric_list = []
    for i in range(1, classes):
