...
def test_single_volume_ensemble(case, net_1, net_2, test_save_path, FLAGS):
    h5f = h5py.File(FLAGS.root_path +
                    "/ACDC_training_volumes/{}".format(case), 'r')
    image = h5f['image'][:]
    label = h5f['label'][:]
    ...
    for ind in range(image.shape[0]):
        ...
        net_2.eval()
        with torch.no_grad():
            out_main1 = torch.softmax(net_1(input)['seg'], dim=1)
            out_main2 = torch.softmax(net_2(input)['seg'], dim=1)
...
def Inference(FLAGS):
    ...
    test_save_path_1 = snapshot_path+"/{}_predictions_net_1/".format(
    ...
    test_save_path_2 = snapshot_path+"/{}_predictions_net_2/".format(
        FLAGS.exp, FLAGS.fold, FLAGS.sup_type, FLAGS.model)
    ...
    net_1 = UNetTF(in_channels=1, classes=FLAGS.num_classes).cuda()
    net_2 = UNetTF(in_channels=1, classes=FLAGS.num_classes).cuda()

    save_mode_path_1 = os.path.join(
        snapshot_path, f'best_net_1_dice.pth')
    ...
    save_mode_path_2 = os.path.join(
        snapshot_path, f'best_net_2_dice.pth')
    ...
    for case in tqdm(image_list):
        ...
        first_metric, second_metric, third_metric = test_single_volume(
            case, net_1,'seg', test_save_path_1, FLAGS)
    ...
    logging.info('inference from net_1.')
    ...
    for case in tqdm(image_list):
        print(case)
        first_metric, second_metric, third_metric = test_single_volume(
            case, net_2,'seg', test_save_path_2, FLAGS)
        first_class_total += np.asarray(first_metric)
        second_class_total += np.asarray(second_metric)
        third_class_total += np.asarray(third_metric)
    avg_metric = [first_class_total / len(image_list), second_class_total /
                  len(image_list), third_class_total / len(image_list)]
    logging.info('inference from net_2.')
    logging.info(avg_metric)
    logging.info((avg_metric[0] + avg_metric[1] + avg_metric[2]) / 3)
    ...
    for case in tqdm(image_list):
        ...
        first_metric, second_metric, third_metric = test_single_volume_ensemble(
            case, net_1, net_2, test_save_path_3, FLAGS)
        first_class_total += np.asarray(first_metric)
        second_class_total += np.asarray(second_metric)
        third_class_total += np.asarray(third_metric)
    avg_metric = [first_class_total / len(image_list), second_class_total /
