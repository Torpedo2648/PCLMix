...
def parser_args():
    ...
    parser.add_argument('--sup_m_weight', type=float, default=1.0, help='hyperparameter')
    parser.add_argument('--unsup_m_weight', type=float, default=1.0, help='hyperparameter')
    parser.add_argument('--contrast_weight', type=float, default=0.13, help='hyperparameter')
    parser.add_argument('--contrast_iter', type=int, default=1000, help='hyperparameter')
    parser.add_argument('--tf_decoder_weight', type=float, default=1.0, help='hyperparameter')
    parser.add_argument('--het_weight', type=float, default=1.0, help='hyperparameter')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(train_dataloader):
            ...
            # pseudo_label[uncertainty_mask] = -1

            #=================== contrast loss (below) ========================
            
            if iter_num > args.contrast_iter and args.contrast_weight > 0.0:
                ...
                preds = torch.argmax(F.softmax(output_cnn, dim=1), dim=1, keepdim=True).to(torch.float)
            ...
            # loss_unsup_mixed_cnn = partial_ce_loss(output_mixed_cnn, pseudo_label)
            ...
            # loss_unsup_mixed_tf = partial_ce_loss(output_mixed_tf, pseudo_label)
            # cross entropy with mixed scribble
            loss_sup_mixed_tf = partial_ce_loss(output_mixed_tf, scribble_mixed[:, 0].long())

            loss_cnn = loss_sup_no_mix_cnn + args.sup_m_weight * loss_sup_mixed_cnn + args.unsup_m_weight * loss_unsup_mixed_cnn
            loss_tf = loss_sup_no_mix_tf + args.sup_m_weight * loss_sup_mixed_tf + args.unsup_m_weight * loss_unsup_mixed_tf
            het_weight = get_current_het_weight(epoch_num, max_iterations, args.het_weight)
            con_weight = args.contrast_weight * (iter_num / max_iterations) ** 0.9
            loss_het = torch.mean((output_cnn - output_tf) ** 2) + torch.mean((output_mixed_cnn - output_mixed_tf) ** 2)
            loss = loss_cnn + args.tf_decoder_weight * loss_tf + con_weight * loss_contra + het_weight * loss_het
            ...
            # print and write
            logging.info('iter {}, lr {:.8f}, loss {:.6f}, loss_cnn {:.6f}, loss_tf {:.6f}, loss_contra {:.6f}, loss_het {:.6f}'.\
                         format(iter_num, lr_, loss.item(),loss_cnn.item(), loss_tf.item(), loss_contra, loss_het.item()))
            ...
            writer.add_scalar('train/loss_cnn', loss_cnn.item(), iter_num)
            writer.add_scalar('train/loss_sup_no_mix_cnn', loss_sup_no_mix_cnn.item(), iter_num)
            writer.add_scalar('train/loss_sup_mixed_cnn', loss_sup_mixed_cnn.item(), iter_num)
            writer.add_scalar('train/loss_unsup_mixed_cnn', loss_unsup_mixed_cnn.item(), iter_num)

            writer.add_scalar('train/loss_tf', loss_tf.item(), iter_num)
            writer.add_scalar('train/loss_sup_no_mix_tf', loss_sup_no_mix_tf.item(), iter_num)
            writer.add_scalar('train/loss_sup_mixed_tf', loss_sup_mixed_tf.item(), iter_num)
            writer.add_scalar('train/loss_unsup_mixed_tf', loss_unsup_mixed_tf.item(), iter_num)
